...
PY
    ...
    assert_true "[[ \$compact_ops_banner == *'Operations'* && \$compact_ops_banner == *'actionstate[/][bold#f59e0b]Attention'* && \$compact_ops_banner == *'ThisView[/]Reviewthemaintenancequeue,hosthousekeepingposture,andupdatepreflightfacts.'* ]]" "dashboard operations banner frames the screen as queue plus housekeeping and update preflight" || all_passed=1
    ...
    assert_true "[[ \$compact_logs == *'HostHousekeepingPosture'* && \$compact_logs == *'Logrotation,filehygiene,anddiskheadroomthatcanaffectmaintenanceconfidence.'* && \$compact_logs == *'Overall[/][bold#34d399]healthy[/]'* && \$compact_logs == *'Queue[/]1scheduledtask'* && \$compact_logs == *'CreateHere[/][bold#f59e0b]h[/]maintenance'* && \$compact_logs == *'Support[/][bold#f59e0b]T[/]testlogs'* ]]" "dashboard operations support panel stays honest about housekeeping posture and maintenance creation" || all_passed=1
    assert_true "[[ \$compact_schedule_intro == *'CreateHere[/][bold#f59e0b]h[/]maintenance'* && \$compact_schedule_intro == *'QueueSource[/]jobsappearhereafterdashboardcreationorthematching[bold#f59e0b]schedule[/]CLIcommands.'* && \$compact_schedule_intro == *'Now[/]1scheduledtaskinthequeue.'* ]]" "dashboard scheduled-maintenance intro explains creation paths and queue provenance" || all_passed=1
