...
snapshot = {
    ...
    "server": {
        ...
        "data": {
            "services": {
                "auth": {"health": "active", "pid": 111, "uptime_human": "1h 0m", "cpu_percent": 0.5, "memory_mb": 20, "restart_count_1h": 0},
                "world": {"health": "warning", "pid": 222, "uptime_human": "2h 0m", "cpu_percent": 12.5, "memory_mb": 512, "restart_count_1h": 1},
            },
            "checks": {
            ...
            "host": {
                ...
                "memory": {"used_percent": 48.0, "status": "ok", "used_kb": 3145728, "total_kb": 6291456},
                "load": {"load_1": 0.62, "load_5": 0.55, "load_15": 0.44, "status": "ok"},
            },
            "storage_io": {"available": True, "util_percent": 33.0, "status": "ok", "device": "sda", "read_ops_per_sec": 1.4, "write_ops_per_sec": 2.6},
            "alerts": {
                "status": "healthy",
                "active": [],
                "recent_events": [
...
payload = {
    ...
    "load_warning_percent": module.normalize_load_percent(8, 8),
    "load_critical_percent": module.normalize_load_percent(16, 8),
...
PY
    ...
    load_warning_percent=$(printf '%s' "$output" | python3 -c 'import json,sys; print(json.load(sys.stdin)["load_warning_percent"])')
    ...
    assert_true "[[ \$compact_banner == *'VMaNGOSManager'* && \$compact_banner == *'realmconsole'* && \$compact_banner == *'Accounts'* && \$compact_banner == *'lastrefresh[/]'* && \$compact_banner == *'lastaction[/][bold#34d399]Complete'* && \$compact_banner == *'—[/]accountcreatePLAYERONE:createdaccount'* && \$compact_banner == *'ThisView[/]Createaccounts;selectoneforpassword,GM,ban,andunban.'* && \$compact_banner == *'Receipt[/]CreatedaccountPLAYERONE.'* && \$compact_banner == *'Next[/]StayinAccountstosetGMaccessorresetthepasswordagain.'* ]]" "dashboard action banner exposes app framing, active view, and stronger receipt feedback" || all_passed=1
    assert_true "[[ \$compact_monitor_banner == *'Monitor'* && \$compact_monitor_banner == *'lastaction[/][bold#7dd3fc]Ready'* && \$compact_monitor_banner == *'ThisView[/]Trends,diskdevicedetail,andper-serviceresourceuse.'* ]]" "dashboard monitor banner frames the deeper diagnostics view explicitly" || all_passed=1
    assert_true "[[ \$compact_ops_banner == *'Operations'* && \$compact_ops_banner == *'lastaction[/][bold#f59e0b]Attention'* && \$compact_ops_banner == *'ThisView[/]Maintenancereadiness,scheduledtasks,andupdateplanning.'* && \$compact_ops_banner == *'Receipt[/]Generatedachange-windowplanandflaggedfollow-upworkbeforecodechanges.'* ]]" "dashboard operations banner frames the screen as maintenance and change-window workflow with receipt context" || all_passed=1
    assert_true "[[ \$compact_cross_view_banner == *'Monitor'* && \$compact_cross_view_banner == *'lastaction[/][bold#34d399]Complete'* && \$compact_cross_view_banner == *'—[/]accountcreatePLAYERONE:createdaccount'* && \$compact_cross_view_banner != *'Receipt[/]'* && \$compact_cross_view_banner != *'Next[/]'* ]]" "dashboard banner keeps global action state but hides follow-up guidance outside the originating view" || all_passed=1
    assert_true "[[ \$compact_sidebar == *'VMaNGOSManager'* && \$compact_sidebar == *'Monitor'* && \$compact_sidebar == *'RealmPulse'* && \$compact_sidebar == *'Updated[/]'* && \$compact_sidebar == *'Players[/]1online'* && \$compact_sidebar == *'Backups[/]3stored'* && \$compact_sidebar != *'operatorconsole'* && \$compact_sidebar != *'ActiveHotkeys'* ]]" "dashboard sidebar stays focused on navigation and realm pulse while exposing the Monitor module" || all_passed=1
    ...
    assert_true "[[ \$monitor_pressure_line_count -le 13 ]]" "dashboard pressure deck stays compact enough for the shipped monitor layout" || all_passed=1
    assert_true "[[ \$compact_monitor_processes == *'RealmProcesses'* && \$compact_monitor_processes == *'Auth[/][bold#2dd4bf]active[/]'* && \$compact_monitor_processes == *'World[/][bold#f59e0b]warning[/]'* && \$compact_monitor_processes == *'cpu[/]12.5%'* && \$compact_monitor_processes == *'mem[/]512MB'* && \$compact_monitor_processes == *'crashloop[/][bold#cbd5e1]no[/]'* && \$compact_monitor_processes == *'DB[/][bold#22c55e]ok[/]'* ]]" "dashboard monitor process panel surfaces auth/world footprint and DB readiness" || all_passed=1
    assert_true "[[ \$compact_monitor_trends == *'Trends'* && \$compact_monitor_trends == *'Nowvswindowpeakvsdirection.'* && \$compact_monitor_trends == *'CPU[/][#cbd5e1]now[/]31.5%'* && \$compact_monitor_trends == *'peak[/]31.5%'* && \$compact_monitor_trends == *'Memory[/][#cbd5e1]now[/]48.0%'* && \$compact_monitor_trends == *'I/O[/][#cbd5e1]now[/]33.0%'* ]]" "dashboard monitor trends panel compares live values against the recent window" || all_passed=1
    assert_true "[[ \$compact_monitor_storage == *'Storage[/]'* && \$compact_monitor_storage == *'FS[/]n/a[#cbd5e1]used[/]24%'* && \$compact_monitor_storage == *'Device[/]sda'* && \$compact_monitor_storage == *'Ops/s[/]read1.4write2.6'* && \$compact_monitor_storage == *'Await[/]0ms'* && \$compact_monitor_storage == *'I/O[/][bold#22c55e]ok[/]'* ]]" "dashboard monitor storage panel exposes device-level detail when iostat data is present" || all_passed=1
    ...
    assert_true "[[ \$compact_realm_logs == *'RealmLogs'* && \$compact_realm_logs == *'Recentauth/worldevents;eachrefreshre-readsthecurrentfilters.'* && \$compact_realm_logs == *'source=allwindow=15mseverity=warninglimit=25'* && \$compact_realm_logs == *'Events[/]2visible'* && \$compact_realm_logs == *'TopSeverity[/]error=1warning=1'* && \$compact_realm_logs == *'BySource[/]auth=1world=1'* && \$compact_realm_logs == *'Source[/]journaldjournal'* && \$compact_realm_logs == *'Escalate[/][bold#f59e0b]7[/]Operationsschedulesthefix'* && \$compact_realm_logs != *'Adjust'* ]]" "dashboard logs summary leads with investigation facts and makes the Logs-to-Ops handoff explicit" || all_passed=1
    ...
    assert_true "[[ \$compact_logs == *'MaintenanceReadiness'* && \$compact_logs == *'Logrotation,permissions,anddiskheadroom;checkbeforeamaintenancewindow.'* && \$compact_logs == *'Overall[/][bold#34d399]healthy[/]'* && \$compact_logs == *'Queue[/]1scheduledtask'* && \$compact_logs == *'Safety[/]liverotationandtimerchangesareCLI/root;thispanelonlyreadsstate.'* && \$compact_logs != *'Support'* ]]" "dashboard maintenance readiness panel stays honest about scheduled-work prep and cross-view boundaries" || all_passed=1
    assert_true "[[ \$compact_schedule_intro == *'Queue[/]1scheduledtaskloadedfromsystemdtimers.'* && \$compact_schedule_intro != *'Source[/]'* ]]" "dashboard scheduled-task intro explains creation paths and queue provenance" || all_passed=1
    assert_true "[[ \$compact_update == *'ChangeWindowReadiness'* && \$compact_update == *'Howfarbehindorigin,DBimpact,andtheplannedsteps.'* && \$compact_update == *'Assessment[/][bold#f59e0b]schemamigrationspending'* && \$compact_update == *'PlanSnapshot'* && \$compact_update == *'Next[/]vmangos-managerbackupnow--verify'* && \$compact_update == *'Safety'* && \$compact_update == *'Apply[/]updatesrunfromtheshell'* ]]" "dashboard update panel stays focused on change-window preflight and makes the CLI boundary explicit" || all_passed=1
    assert_true "[[ \$compact_schedule == *'SelectedTask'* && \$compact_schedule == *'Whatthistaskis,whenitrunsnext,andhowtocancelit.'* && \$compact_schedule == *'Queue[/]1scheduledtask'* && \$compact_schedule == *'TaskID[/]20260413220000-1111'* && \$compact_schedule == *'Type[/]restart'* && \$compact_schedule == *'Origin[/]restartwindowscheduledbyManager'* && \$compact_schedule != *'Create[/]'* && \$compact_schedule == *'NextAction[/][bold#f59e0b]j[/]removethistaskifitisnolongerdesired.'* && \$compact_schedule != *'OpsResult'* ]]" "dashboard selected-task details stay scoped to queue inspection, origin, and next action" || all_passed=1
